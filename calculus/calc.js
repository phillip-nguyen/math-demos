...
CALC = (function() {
	...
	while (input) {
	    ...
	    if (find_next_match("FUNCTION", /^(sin|cos|tan|csc|sec|cot|ln|log|exp|abs|floor|ceil|sqrt|arcsin|arccos|arctan)/)) continue;
	...
	var f = {"sin": Math.sin, "cos": Math.cos, "tan": Math.tan, 
		 ...
		 "log": Math.log10, "sqrt": Math.sqrt, "floor": Math.floor,
		 "ceil": Math.ceil, "arcsin": Math.asin, "arccos": Math.acos,
		 "arctan": Math.atan};
	// Table of constants.
	var c = {"pi": [Math.PI, 1], "e": [Math.E, 1]};
	var a, b, tok;
	...
	for (var i = 0, len = postfix.length; i < len; i++) {
	    tok = postfix[i];
	    if (tok.type === 'NUMBER') {
		result.push(MATH.rat_simplify([parseFloat(tok.token), 1]));
		...
		a = result.pop();
		result.push(MATH.rat_neg(a));
	    } else if (tok.type === '+') {
		b = result.pop();
		a = result.pop();
		result.push(MATH.rat_add(a, b));
	    } else if (tok.type === '-') {
		b = result.pop();
		a = result.pop();
		result.push(MATH.rat_sub(a, b));
	    } else if (tok.type === '*') {
		b = result.pop();
		a = result.pop();
		result.push(MATH.rat_mul(a, b));
	    } else if (tok.type === '/') {
		b = result.pop();
		a = result.pop();
		result.push(MATH.rat_div(a, b));
	    } else if (tok.type === '^') {
		b = result.pop();
		a = result.pop();
		result.push(MATH.rat_pow(a, b));
	    } else if (tok.type === 'FUNCTION') {
		a = result.pop();
		result.push([f[tok.token](MATH.rat_real(a)),1]);
	    }
	}
	// Last element in the evaluation stack is the result,
	// so we return it.
	return MATH.rat_real(result.pop());
    }
	...
	try {
	    postfix = parse(input);
	    return function(val) { return evaluate_postfix(postfix, MATH.rat_simplify([val, 1])); }
	} catch (exception) {
	    return null;
	}
    }
